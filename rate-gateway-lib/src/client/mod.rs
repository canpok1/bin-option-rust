    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn rates_pair_post(
        &self,
        param_pair: String,
        param_rate: &Vec<models::Rate>,
        context: &C) -> Result<RatesPairPostResponse, ApiError>
    {
        ...
        // Body parameter
        let body = serde_json::to_string(&param_rate).expect("impossible to fail to serialize");
        ...
        match response.status().as_u16() {
            201 => {
                ...
                let body = serde_json::from_str::<models::PostSuccess>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(RatesPairPostResponse::Status201
                    (body)
                )
            }
            400 => {
                ...
                Ok(RatesPairPostResponse::Status400
                    (body)
                )
            }
            404 => {
                ...
                Ok(RatesPairPostResponse::Status404
                    (body)
                )
            }
            500 => {
                ...
                Ok(RatesPairPostResponse::Status500
                    (body)
                )
            }
            code => {
